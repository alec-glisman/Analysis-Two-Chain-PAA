...
Date: 2023-03-30
Description: Native contacts matrix calculation.
...
This is essentially identical to the :class:`colvar_parallel.contacts.Contacts`
but we return each contact number instead of the contact number summed
for each of the reference atoms. This is useful for calculating the
contact matrix distance.
...
def linear(r: float, r0: float, d0: float = 0.0) -> float:
    """
    Simply returns the linear distance between two points, r.
...
class ContactMatrix(ParallelAnalysisBase):
    ...
    **Contact API** prescribes that this method must be a function with call
    ...
    def __init__(
        ...
        **basekwargs,
        ...
        method : string | callable (optional)
            Can either be one of ``["linear", "hard_cut" , "soft_cut", "radius_cut", "rational", "6_12"]``
        ...
        if method == "hard_cut":
            ...
            self._fraction_contacts = linear
        else:
        ...
        self._ag1 = self._get_atomgroup(u, select[0], updating=False)
        self._ag2 = self._get_atomgroup(u, select[1], updating=False)
        self._radius = radius

        # output data
        self._dir_out: Path = Path(f"./mdanalysis_contact_matrix")
        self._df_filename = f"contact_matrix_{self._tag}.parquet"
        self._logger.debug(f"df_filename: {self._df_filename}")
        self._df = None
        self._columns = ["frame", "time"]
        for i in range(len(self._ag1)):
            for j in range(len(self._ag2)):
                self._columns.append(f"ag1_{i}_ag2_{j}")
    ...
    def _single_frame(self, idx_frame: int) -> np.array:
        ...
        results = np.empty(2 + n1 * n2, dtype=np.float64)
        results.fill(np.nan)
        ...
        dists = distance_array(
            ag1.positions, ag2.positions, box=ts.dimensions
        )
